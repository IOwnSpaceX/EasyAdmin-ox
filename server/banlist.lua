...
RegisterServerEvent("EasyAdmin:banPlayer", function(playerId,reason,expires)
    if playerId ~= nil and CheckAdminCooldown(source, "ban") then
        if (DoesPlayerHavePermission(source, "player.ban.temporary") or DoesPlayerHavePermission(source, "player.ban.permanent")) and CachedPlayers[playerId] and not CachedPlayers[playerId].immune then
            ...
            updateBlacklist( ban )
			Storage.addAction("~r~BANNED~w~~s~", CachedPlayers[playerId].discord, reason, getName(source, true), CachedPlayers[source] and CachedPlayers[source].discord or "Unknown", ban.banid, ban.expireString)
...
function addBanExport(playerId,reason,expires,banner)
    ...
    updateBlacklist( ban )
	Storage.addAction("~r~BANNED~w~~s~", CachedPlayers[playerId] and CachedPlayers[playerId].discord or nil, reason, banner or "Unknown", "Unknown", ban.banid, ban.expireString)
